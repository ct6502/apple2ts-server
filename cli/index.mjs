...
Global options:
  ...
  --skip-health-check    Skip integrated-server preflight check
  --help                 Show help
...
const assertIntegratedServer = async (context) => {
  if (context.skipHealthCheck) {
    return
  }

  const healthUrl = buildUrl(context.server, "/api/health")
  ...
  try {
    ...
    fail(
      `Cannot reach integrated server at ${context.server}. ` +
        `Use --server to point to the Apple2TS integrated server, or use --skip-health-check to bypass this preflight. ` +
  ...
  if (!response.ok || !contentType.includes("application/json")) {
    fail(
      `Server preflight failed for ${context.server}. Expected Apple2TS integrated server at /api/health, ` +
        `but got HTTP ${response.status} with content type ${contentType || "unknown"}. ` +
        "This usually means you are pointing at a different dev server.",
    )
  }
  ...
  try {
    ...
    fail(
      `Server preflight failed for ${context.server}. /api/health did not return valid JSON. ` +
        "This usually means you are pointing at a different dev server.",
    )
  }
  ...
  if (!payload || typeof payload !== "object" || payload.status !== "ok") {
    fail(
      `Server preflight failed for ${context.server}. /api/health returned an unexpected payload. ` +
        "Use --skip-health-check if this is intentional.",
    )
  }
}
...
const apiRequest = async (context, method, pathname, { query, body, raw = false } = {}) => {
  ...
  if (!response.ok) {
    if (payload && typeof payload === "object" && payload.error && typeof payload.error === "object") {
      const errorCode = payload.error.code || "ERROR"
      const errorMessage = payload.error.message || "Request failed"
        const emulatorUrl = new URL("/?remoteControl=1", context.server).toString()
      const missingClientHint =
        (response.status === 503 && errorCode === "NO_CONNECTED_CLIENT") ||
        (response.status === 404 && errorCode === "NOT_FOUND")
            ? `\nHint: no browser emulator client appears connected. Start the server, open ${emulatorUrl} in a browser, then retry.`
          : ""
      fail(`HTTP ${response.status}: ${errorCode}: ${errorMessage}${missingClientHint}`)
    }
...
const main = async () => {
  ...
  await assertIntegratedServer(context)
